...
//https://moliam.space/2021/11/17/experience/esp32/adc_dma/
// copy, xiaolaba 2022-OCT-14

//ref https://github.com/espressif/esp-idf/blob/master/components/driver/test/dac_dma_test/test_esp32s2.c


//xiaolaba
#include "esp32_dma_adc.h"
// Arduino IDE ESP32, path, C:\Users\user0\Desktop\esp-idf-v4.3\components\soc\esp32\include\soc
#include "soc/spi_reg.h"  // SPI_DMA_INT_ST_REG
//C:\Users\user0\AppData\Local\Arduino15\packages\esp32\hardware\esp32\2.0.5\tools\sdk\esp32\include\soc\esp32\include\soc
#include "soc/periph_defs.h"          // ETS_SPI3_DMA_INTR_SOURCE, esp32-s2 only
#include "hal/adc_hal.h"      // adc_digi_config_t
...
*/
    dma1 = lldesc_t {};
        dma1.size = SAR_SIMPLE_NUM*2*2;
        dma1.owner = 1;
        dma1.buf = &link_buf[0];
        dma1.qe.stqe_next = NULL;
...
//    adc_dac_dma_isr_handler_t* item = malloc(sizeof(*item));
    adc_dac_dma_isr_handler_t * item = (adc_dac_dma_isr_handler_t*) malloc(sizeof(*item));
...
{
/*  
    adc_digi_config_t config = {  //'adc_digi_config_t' was not declared in this scope
        .conv_limit_en = false,
        .conv_limit_num = 0,
        .interval = 40,
...
*/
  ...
  // port to Arduino IDE and compile, xiaolaba
    adc_digi_config_t config = {};
        ...
        config.dma_eof_num = SAR_SIMPLE_NUM*2;
